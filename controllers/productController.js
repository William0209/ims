  ...
  try {
    ...
    if (!product) {
      return res.status(404).json({
        status: "fail",
        message:
          "Ingen produkt hittades med det ID:t",
      });
    }
    res.status(200).json({
      status: "success",
...
exports.getTotalStockValue = async (
  req,
  res
) => {
  try {
...
exports.getTotalStockValueByManufacturer =
  async (req, res) => {
    try {
      const products =
        await Product.find();
      // Gruppera och summera värdet av produkter per tillverkare
      const totalByManufacturer =
        products.reduce(
          (acc, product) => {
            if (
              !acc[
...
exports.getCriticalStockProducts =
  async (req, res) => {
    try {
      // Hitta produkter med mindre än 5 i lager och returnera mer information
      const products =
        await Product.find({
          amountInStock: { $lt: 5 },
        }).select(
          "name sku price manufacturer.contact.name manufacturer.contact.phone manufacturer.contact.email"
        );
      res.status(200).json({
        status: "success",
        data: {
